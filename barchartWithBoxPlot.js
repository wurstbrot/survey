...
barchartWithBoxPlot = function (containerId, width, height, data) {
    ...
    var barchartMargin = {top: 10, right: 0, bottom: 175, left: 35};
    ...
    function setColumnDetailsAndTotals() {
        data.forEach(function (d) {
            ...
            d.columnDetails = _displayedVariables.map(function (element, i) {
                ...
                console.log("d[elemment.name",d[element.name]);
                */
                var result = {
                    name: element.name,
                    column: index.toString(),
                    yBegin: (0 > d[element.name].value ? d[element.name].value : 0),
                    yEnd: (0 < d[element.name].value ? d[element.name].value : 0),
                    uncertainty: d[element.name].uncertainty,
                    color: false,
                    region: d.Name
                };
                index++;
